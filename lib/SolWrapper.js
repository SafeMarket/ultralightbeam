...
SolWrapper.prototype.getTransactionRequest = function getTransactionRequest(
  solidityMethodName, inputs, _options
) {
  const options = _options || {}
  const data = this.getCalldata(solidityMethodName, inputs)
  return new TransactionRequest(_.merge({ data, to: this.address }, options))
}

SolWrapper.prototype.broadcast = function broadcast(solidityMethodName, inputs, options) {
  const transactionRequest = this.getTransactionRequest(solidityMethodName, inputs, options)
...
function EventLog(log, abi) {
  ...
  if (item === null) {
    throw new errors.TransactionReceiptParseError()
  }
