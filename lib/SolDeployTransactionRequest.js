...
function SolDeployTransactionRequest(ultralightbeam, bytecode, abi, inputs, _transactionRequestOptions) {
  arguguard('SolDeployTransactionRequest', [Object, Amorph, Array, Array, Object], arguments)

  const encodedParams = encodeConstructorParams(abi, inputs)
  const data = new Amorph(bytecode.to('array').concat(encodedParams.to('array')), 'array')
