...
const Solquest = require('./Solquest')
...
Solbuilder.prototype.deploy = function deploy (_inputs) {
  ...
  return new Solquest('eth', 'sendTransaction', [new TransactionRequest({ data })])
}
...
Solbuilder.prototype.get = function get (address, solidityFunctionName, _inputs) {
  ...
  return new Solquest('eth', 'call', [new TransactionRequest({ data, to: address }), blockFlags.latest])
}
