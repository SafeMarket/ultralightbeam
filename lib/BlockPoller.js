  ...
  this.setupInterval = setInterval(() => {
    ...
    blockBatchPart.deferred.promise.then((block) => {
      if (
        ...
        // fill gaps
        if (oldBlock) {
          const oldBlockNumber = oldBlock.number.to(amorphNumber.unsigned)
          const gapSize = block.number.to(amorphNumber.unsigned) - oldBlockNumber
          gapFillPromise = Promise.all(_.range(gapSize - 1).map((index) => {
            return this.ultralightbeam.eth.getBlockByNumber(Amorph.from(amorphNumber.unsigned, oldBlockNumber + index + 1), true)
          }))
        } else {
          gapFillPromise = this.ultralightbeam.resolve([])
        }

        gapFillPromise.then((gapBlocks) => {
          gapBlocks.forEach((gapBlock) => {
            this.emitter.emit('block', gapBlock)
          })
