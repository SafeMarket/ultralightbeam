...
const Account = module.exports = defunction([v.privateKey], v.undefined, function Account(privateKey) {
  ...
  const privateKeyBuffer = new Buffer(privateKey.uint8Array)

  const uncompressedPublicKeyUint8Array = new Uint8Array(secp256k1.publicKeyCreate(privateKeyBuffer, false))
  const compressedPublicKeyUint8Array = new Uint8Array(secp256k1.publicKeyCreate(privateKeyBuffer, true))
...
Account.generate = defunction([], v.account, function generate() {
  ...
  } while (!secp256k1.privateKeyVerify(new Buffer(privateKey.uint8Array)))
