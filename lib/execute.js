const _ = require('lodash')
const Batch = require('./Batch')
const NoResultReturnedError = require('./errors/NoResultReturned')
const defunction = require('defunction')
const v = require('./validates')
const FailedTransactionError = require('./errors/FailedTransaction')
const InvalidResultError = require('./errors/InvalidResultError')

module.exports = defunction([v.ultralightbeam], v.anything, function execute(ultralightbeam) {

  const batch = ultralightbeam.batch

  ultralightbeam.batch = new Batch(ultralightbeam)
  ultralightbeam.batches.push(batch)

  const payloads = batch.parts.map((batchPart) => {

    batchPart.id = ultralightbeam.id++

    const inputs = batchPart.inputs
    const inputter = batchPart.interface.inputter
    const params = inputter ? inputter.apply(ultralightbeam, inputs) : inputs

    return {
      jsonrpc: '2.0',
      id: batchPart.id,
      method: batchPart.interface.method,
      params
    }

  })

  ultralightbeam.provider.sendAsync(payloads, (err, results) => {

    if (err && !results) {
      batch.parts.forEach((batchPart) => { batchPart.deferred.reject(err) })
      batch.deferred.reject(new Error(err.message))
      return
    }

    batch.parts.forEach((batchPart) => {

      const result = _.find(results, { id: batchPart.id })

      if (!result) {
        const error = new NoResultReturnedError(batchPart.id)
        batchPart.deferred.reject(error)
        return
      }

      if (result.error) {
        batchPart.deferred.reject(new FailedTransactionError(result.error.message))
        return
      }

      try {
        const outputter = batchPart.interface.outputter
        const output = outputter ? outputter.apply(ultralightbeam, [result.result]) : result.result
        batchPart.deferred.resolve(output)
      } catch(err) {
        const error = new InvalidResultError(batchPart.id)
        batchPart.deferred.reject(error)
      }

    })

  })

  return batch.promise
})
